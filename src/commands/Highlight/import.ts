...
export default class extends Command {
	...
	async words(message: KlasaMessage, [guild]: [KlasaGuild]) {
		...
		if (addedWords.length) {
			embed.setTitle(`The following ${pluralize(addedWords.length, 'word', 'words')} ${pluralize(addedWords.length, 'has', 'have')} been added to your list`)
	...
	async regex(message: KlasaMessage, [guild]: [KlasaGuild]) {
		...
		if (added.length) {
			embed
				.setTitle(`The following ${pluralize(added.length, 'expression', 'expressions')} ${pluralize(added.length, 'has', 'have')} been added to your list`)
				// eslint-disable-next-line @typescript-eslint/require-array-sort-compare
